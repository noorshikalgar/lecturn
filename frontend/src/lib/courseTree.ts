...
export function findNodeById(nodes: CourseTreeNode[], id: number): CourseTreeNode | undefined {
  for (const node of nodes) {
...
export function findChapter(tree: CourseTreeNode[], activeId: number): CourseTreeNode | undefined {
...
export function flattenAll(nodes: CourseTreeNode[], map: Map<number, { title: string; orderIndex: number }>) {
  for (const node of nodes) {
